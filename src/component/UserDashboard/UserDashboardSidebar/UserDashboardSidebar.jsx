...
const UserDashboardSidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const menuItems = [
    { icon: <FaSearch />, text: "Search", path: "/dashboard" },
    {
      icon: <FaBuilding />,
      text: "Companies",
      path: "/dashboard/company_list",
    },
    {
      icon: <FaEnvelope />,
      text: "Ai Generator",
      path: "/dashboard/ai_generator",
    },
    {
      icon: <FaAddressBook />,
      text: "Contact list",
  ...
  useEffect(() => {
    ...
    else if (currentPath === "/dashboard/company_list")
      newActiveItem = "Companies";
    else if (currentPath === "/ai-generator") newActiveItem = "Ai Generator";
    else if (currentPath === "/dashboard/contact_list")
      newActiveItem = "Contact list";
    else if (currentPath === "/dashboard/membership")
      newActiveItem = "Membership";
    else if (
      currentPath === "/dashboard/profile" ||
      currentPath === "/dashboard/edit_profile"
    )
      newActiveItem = "Profile";
    else if (currentPath === "/logout") newActiveItem = "Logout";
    else if (currentPath === "/dashboard/company_details")
      newActiveItem = "Search";
    ...
    <div className="py-4 pr-4 mt-20 roboto">
      {menuItems.map((item, index) => (
        <button
          ...
          className={`w-full flex items-center space-x-5 py-[11px] rounded-r-md transition-colors duration-200 my-2 cursor-pointer pl-8 ${
